...
3. 자식 클래스가 태어날 때는 본인 생성자보다 부모의 생성자를 먼저 호출한다.
4. 자식이 상속받아 재정의한 것은 재정의한 것으로 쓰인다.
5. 자식이 태어날 때는 상속 받은 모든 부모의 생성자를 위에서부터 호출한다.
6. 자식이 부모의 생성자를 명시적으로 호출하는 함수는 super()이다.
7. 만약 부모가 생성자가 여러개(오버로딩)이라면, super()안의 매개변수로 조절한다.

1️⃣ 업캐스팅(Upcasting)

- 부모 형태의 자식 생성자를 할당하는 것 (ex. Parent p = new Child(...))
    - 객체지향 프로그래밍에서 서브(자식)클래스의 객체를 슈퍼(부모)클래스 타입 변수에 할당하는 것 (위로 캐스팅한다)


* 업캐스팅의 원칙

- 자식에 이름이 같은 메서드가 있으면 우선 작동함.
- 자식에 super()가 있으면, 부모의 그것을 호출함.
    - super()는 무조건 최상위줄에 있어야 함.
- 부모에 자식과 같은 메서드가 있으면, 자식을 호출함.
- this는 자신을 출력 / 자식에게 파라미터가 있어도 부모는 기본 생성자만 호출 (명시적이지 않을 땐 무조건 기본. 혹시나 super(?)가 있다면 모양이 같은 명시적 생성자를 호출)
    ...
    * 생성자가 있다면?
    ...
    - 자식에 super()가 있으면, 부모의 그것을 호출함.
    - 부모에 자식과 같은 메서드가 있으면, 자식을 호출함.
    - 자식이 태어날 때는 부모가 먼저 태어나고 태어남.

    * 자식 클래스에만 있는 메서드 호출?
    - 무조건 오류. 부모도 함께 가지고 있는 메서드를 사용해야 함.

=> super, this의 위치와 파라미터 주의하기!